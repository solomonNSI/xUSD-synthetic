...
export function MintBurnTipCard() {
  const [show, setShow] = useState(config.showTipBox);
  if (!show) return null;
  return (
    <div className="relative px-3 py-3 w-90 bg-red-500 shadow-lg rounded-xl opacity-95">
      <h2 className="text-white sm:text-lg">Alpha Tip: Try minting xUSD!</h2>
      <div className="flex items-end justify-between ">
        <p className="text-white mt-1.5 text-xs sm:text-sm max-w-[70%]">
          Be a holder of the innovative delta-neutral stablecoin
        </p>
        <a
          href={links.memo}
          target="_blank"
